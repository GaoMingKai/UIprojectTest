...
class Case078(unittest.TestCase):
    ...
    def AddWorkItem(self, driver):
        a = WebDriverTools()
        WebDriverTools.waitElementNotFound(driver,".loadingMask",self.testCaseID,timeout=20)
        if a.isElementPresent(driver, '#wf-main-project-hash'):
            driver.find_element_by_css_selector('#wf-main-project-hash>a').click()
            WebDriverTools.waitElementNotFound(driver,".loadingMask",self.testCaseID,timeout=15)
            sleep(5)
