...
class Case045(unittest.TestCase):
    ...
    def outPutData(self, driver):
        ...
        #先获取所有点表的信息再比对dataMa
        time.sleep(5)
        lines = driver.find_elements_by_css_selector('.table.table-bordered.table-striped tr')[1:]
        org_data = []
        for line in lines:
    ...
    def checkDownLoadList(self, org_data):
        ...
        for index, line in enumerate(sorted(org_data)):
    ...
    def upLoadFile(self, driver, org_data):
        ...
        driver.find_elements_by_css_selector(".pointManagerCloudPointUl")[1].find_elements_by_css_selector("li")[0].click()
        time.sleep(2)
        driver.find_element_by_css_selector('#uploadInput').send_keys(app.config["DOWNLOAD_DIR"]+'/myTest.xlsx')
        #调用exe完成上传工作
