...
projects = (
            (81,'成都时代1号',['运营报表','KPI日报'],'#beopReport','old'),
            (194,'上海印钞厂',['运营报表','运行日报'],'#beopReport','old'),
            (19,'上汽工业',['报表','运行日报'],'#beopReport','old'),
            (364,'某办公楼',['Run Report'],'.reportListName','new'),
            (284,'上实大厦',['运营报表'],'.reportListName','new'),
            (126,'国家会展中心',['运营报表','运行日报'],'#beopReport','old'),
            (318,'金钟广场',['运营报表'],'.reportListName','new'),
...
TIMEOUT = 60
class Case052(unittest.TestCase):
    testCaseID = 'Case052'
    projectName= '早班巡查项目3'
    ...
    def checkOld(self, driver, projectName):
        ...
        for i in range(len(reports)):
            ...
            time.sleep(6)
            WebDriverTools.waitElementNotFound(driver, '.loadingMask', self.testCaseID)
    ...
    def checkNew(self, driver, projectName):
        ...
        for i in range(len(reports)):
            ...
            #WebDriverTools.waitSpinner(driver, projectName + "报表--%s" % driver.find_elements_by_css_selector('.reportListName a')[i].text, timeout=50)
            try:
                WebDriverTools.waitElementNotFound(driver, '.loadingMask', self.testCaseID, timeout=TIMEOUT)
