...
projects = (
            ...
            (72,'上海华为',['设备','系统诊断','诊断周报'],'.center.report-wrap.gray-scollbar','new'),
            ...
            #(76,'中文演示06',["设备",'系统诊断','诊断报表'],'#reportNavList','old'),
...
class Case024(unittest.TestCase):
        ...
        for i in range(len(reports)):
            ...
            if contents != []:
                ...
                if ("月" in reportName or "onth" in reportName or reportName == 'Diagnosis Report' or reportName == '诊断报表' or 'Pattern report' in reportName) and int(datetime.datetime.now().day) <= 5:
                    print("5号之前月报还没生成!")
                elif('周报' in reportName and int(datetime.datetime.today().weekday())==0):
                    print("周报在周一还没有出来")
                else:
    ...
    def downloadPDF(self, driver, dir, project, report, mode):
        ...
        time.sleep(2)
        try:
            WebDriverWait(driver,10).until_not(lambda x:x.find_element_by_id("loadBar"))
        except:
            time.sleep(10)
        #点击下载PDF按钮
        try:
