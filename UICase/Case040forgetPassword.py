...
class Case040(unittest.TestCase):
    testCaseID = 'Case040'
    projectName = "不针对项目"
    buzName = '3289525927@qq.com账号找回密码'
    start = 0.0
    now = 'None'
    startTime = ""
    ...
    def Test(self):
        ...
        driver.find_element_by_css_selector("#inputEmail").send_keys("3289525927@qq.com")
    ...
    def CheckEmail(self):
        ...
        time.sleep(5)
        dr.find_element_by_css_selector("#uinArea>div>input").send_keys("3289525927@qq.com")
        dr.find_element_by_css_selector("#pwdArea>div>input").send_keys("gmk4335482.")
        ...
        try:
            ...
            assert 0,"用户点击忘记密码输入确认后->用户打开邮件后未进入到注册页面,因为页面中没有找到密码输入框!(等待10秒)"
            dr.close()

        dr.find_element_by_css_selector("#inputPassword1").clear()
        dr.find_element_by_css_selector("#inputPassword1").send_keys("gmk123456789")
        dr.find_element_by_css_selector("#inputPassword2").clear()
        dr.find_element_by_css_selector("#inputPassword2").send_keys("gmk123456789")
