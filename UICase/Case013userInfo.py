...
class Case013(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.driver = lg.login(self.driver, 'tester5')
    ...
    def Test(self):
        ...
        try:
            ...
            self.error.append("登陆Beop后用户菜单按钮消失！")
        time.sleep(2)
        ...
        if user_now == TestName:
            ...
            driver.get_screenshot_as_file(r'.\ErrorPicture\%s\%s.png' % (self.testCaseID,time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())))
            self.error.append('用户信息->修改用户名失败!')
        #将用户名还原
        time.sleep(1.8)
        #driver.find_element_by_id("editName").click()
        driver.find_element_by_id("editInput").clear()
        driver.find_element_by_id("editInput").send_keys(user.split("@")[0])
        driver.find_element_by_id("confrimEditName").click()
        time.sleep(2)
        #查询用户今天的登录记录
        ...
        if len(record):
            ...
            self.error.append("用户信息->查询今日登录信息无结果,请检查!")
        time.sleep(2)
        #修改用户密码
        driver.find_element_by_css_selector("a[data-shown='loginPwd']").click()
        time.sleep(1)
        driver.find_element_by_id("editOldPwd").clear()
        driver.find_element_by_id("editOldPwd").send_keys("RNB.beop-2013")
        driver.find_element_by_id("editNewPwd").clear()
        driver.find_element_by_id("editNewPwd").send_keys("gmk123456")
        driver.find_element_by_id("editCheckNewPwd").clear()
        driver.find_element_by_id("editCheckNewPwd").send_keys("gmk123456")
        driver.find_element_by_id("submitResetPwd").click()
        time.sleep(0.3)
        if a.isElementPresent(driver,'.ant-notification-notice-message'):
            print(driver.find_element_by_css_selector('.ant-notification-notice-message').text)
        else:
            self.error.append("用户%s修改密码后未弹出修改成功提示!请检查密码是否修改失败!"%user)
        ...
        try:
            ...
            driver.find_element_by_id("txtName").send_keys("projecttest_pwdreset@rnbtech.com.hk")
            driver.find_element_by_id("txtPwd").clear()
            driver.find_element_by_id("txtPwd").send_keys("gmk123456")
            driver.find_element_by_id("btnLogin").click()
            a.waitSpinner(driver,'修改密码之后登陆','loadingMask')
            time.sleep(1)
            print("修改成功")
        except Exception:
            driver.get_screenshot_as_file(r'.\ErrorPicture\%s\%s' % (self.testCaseID,time.strftime("%Y-%m-%d %H.%M.%S", time.localtime())))
            self.error.append("登陆失败,修改密码失败!")
        ...
        try:
            ...
            driver.get_screenshot_as_file(r'.\ErrorPicture\%s\%s.png' % (self.testCaseID,time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())))
            self.error.append("修改密码--注销之后再次登陆失败！")
        time.sleep(0.8)
        driver.find_element_by_id("paneUser").click()
        time.sleep(1.5)
        driver.find_element_by_css_selector("a[data-shown='loginPwd']").click()
        ...
        try:
            driver.find_element_by_id("editOldPwd").clear()
            driver.find_element_by_id("editOldPwd").send_keys("gmk123456")
            driver.find_element_by_id("editNewPwd").clear()
            driver.find_element_by_id("editNewPwd").send_keys("RNB.beop-2013")
            driver.find_element_by_id("editCheckNewPwd").clear()
            driver.find_element_by_id("editCheckNewPwd").send_keys("RNB.beop-2013")
            driver.find_element_by_id("submitResetPwd").click()
        except Exception:
            driver.get_screenshot_as_file(r'.\ErrorPicture\%s\%s.png' % (self.testCaseID,time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())))
            self.error.append("修改密码--注销之后再次修改密码--失败！")
        time.sleep(4)
