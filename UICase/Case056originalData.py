...
class Case056(unittest.TestCase):
    ...
    def Test(self):
        ...
        self.tools.enterModuleByUserMenu(driver, 'btnPointManager', '数据管理', '#divPointsList')
    ...
    def checkHistory(self, driver):
        ...
        jionCurve.click()
        try:
            WebDriverWait(driver, 15).until_not(lambda x: x.find_elements_by_class_name('loadingMask'))
            print('历史曲线显示出来')
        ...
        if (overtime - starttime) or overtime == starttime:
            print('刷新按钮管用')
        else:
    ...
    def checkPagination(self, driver):
        ...
        if len(driver.find_elements_by_css_selector('.pagination > li > a')) > 8:
            ...
            driver.find_element_by_css_selector('.pagination > li.next >a').click()
            sleep(2)
            ...
            driver.find_element_by_css_selector('.pagination > li.last >a').click()
            sleep(2)
