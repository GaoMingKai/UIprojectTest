...
class Case034(unittest.TestCase):
    ...
    def newProj(self,driver,projName):
        ...
        try:
            #等待新建项目对话框出现
            WebDriverWait(driver, 15).until(lambda x: x.find_element_by_css_selector(".proName"))
        except Exception as e:
            assert 0,"进入factory后点击新建项目按钮，等待15秒仍未弹出新建项目对话框!"
        #清空项目名称输入框
        sleep(1)
        ...
        if projs != []:
            '''for p in projs:
                if p.text == projName:
                    ...
                    driver.find_element_by_css_selector("#removePass").send_keys("h=Lp4U8+Lp")
                    sleep(1)
                    driver.find_element_by_css_selector(".btn.btn-default.directRemove").click()
                    break'''
