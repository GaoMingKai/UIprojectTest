...
class Case103(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        if "Exception" in text or "AssertionError" in text or self.error!=[]:
    ...
    def Test(self):
        ...
        self.error = []
        driver = self.driver
        tools.enterProject(driver,425,projName=self.projectName,errors=self.error)
    ...
    def checkTime(self,driver,error):
        ...
        WebDriverWait(driver,5).until_not(lambda x:x.find_element_by_css_selector(".loadingMask"))
        exportbutton.click()
        status = True
        try:
            driver.find_element_by_css_selector(".infoBox.infoBox-unique.infoBoxAlert")
            status = False
        except Exception as e:
            pass
        if status:
            WebDriverTools.checkloadexcelFile(self.filename,error,text.text.split("\n"))
        else:
            pass
