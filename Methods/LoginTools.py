...
class LoginTools:
    ...
    def phantomjsLogin(self,case):
        driver = webdriver.PhantomJS()
        if driver:
            print('用例: {}指定浏览器为{}'.format(case,"phantomjs"))
            driver.set_page_load_timeout(8)
            driver.maximize_window()
            try:
                driver.get(url)
            except Exception as e:
                assert "使用phantomjs失败,错误信息:{}".format(e.__str__())
            return driver
        else:
            assert "用例%s执行失败！"%case
    ...
    def loginrem(self, driver):
        ...
        WebDriverTools.waitSpinner(driver, '登陆页面', 'loadingMask')
        time.sleep(5)
        return driver
    ...
    def login(self, browser, user='default'):
        ...
        try:
            WebDriverWait(browser,15).until(lambda x:x.find_element_by_css_selector(".map-scrollbar"))
        except:
            time.sleep(2)
        time.sleep(2)
        ele = False
    ...
    def loginnow(self, driver):
        try:
            ...
            WebDriverTools.waitSpinner(driver, 'Beop首页', 'loadingMask')
            time.sleep(2)
            return driver
        except Exception as e:
            Log.writeLogError(self.logName, e.__str__())
