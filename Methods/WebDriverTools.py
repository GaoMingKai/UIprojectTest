...
class WebDriverTools:
    ...
    @classmethod
    def waitSpinner(self, driver, page, name='loadingMask', timeout=23, implicit=8, mode=0):
        try:
            use = self.findSpinner(driver, name, timeout, mode, implicit, page)
            print("进入%s 消耗时间: %.2f秒" % (page, abs(use)))
            return use
        except Exception as e:
    ...
    def enterPage(self, driver, pageName, assertResult=None, projName='', timeout=30):
        try:
            ...
            for index, page in enumerate(pageName):
                ...
                sleep(1)
            WebDriverWait(driver, timeout).until_not(lambda x: x.find_element_by_class_name('.loadingMask'))
            # assertResult为判断页面是否出来的元素
            if assertResult:
                try:
                    WebDriverWait(driver, timeout).until(lambda x: x.find_element_by_css_selector(assertResult))
                except:
                    assert 0,"%s项目--%s页面加载失败！"%(projName,'--'.join(pageName))
    ...
    def checkCanvas(self,driver,errors,page):
        try:
            spinner = WebDriverWait(driver, 10).until_not(lambda x: x.find_element_by_class_name('loadingMask'))
            if (spinner):
    ...
    def CheckoperationReport(self,driver,projectName,error,timeout=10):
        spinner = True
        time = (datetime.datetime.now()-datetime.timedelta(days=1)).day
        a=WebDriverTools()
        ...
        try:
            status=driver.find_elements_by_css_selector(".datetimepicker-days>.table-condensed>tbody>tr>td[class='day']")[-2]
        except:
            status = False
        try:
            if status:
                ...
                driver.find_element_by_css_selector(".datetimepicker-days>.table-condensed>tbody>tr>td[class='day active']").click()
        except Exception as e:
            print(e.__str__())
        ...
        for index,ele in enumerate(leftare):
            if index != 0:
                ele.click()
            try:
                WebDriverWait(driver,20).until_not(lambda x:x.find_element_by_css_selector("#loadBar"))
            except Exception as e:
                spinner = False
                print(e.__str__())
                error.append("%s中的%s加载20s页面未加载成功！"%(projectName,ele.text.split("\n")[0]))
            if spinner:
                self.checkDownload(driver,error,ele.text.split("\n")[0],projectName)
                self.checkDownload(driver,error,ele.text.split("\n")[0],projectName,type=1)
                self.checkDownload(driver,error,ele.text.split("\n")[0],projectName,type=2)
    ...
    #文件下载type=o为pdf为1为docx,type为2的时候为Eecel
    @classmethod
    def checkDownload(self,driver,error,filename,projectName,type=0):
        ...
        status = False
        try:
            WebDriverWait(driver,20).until_not(lambda x:x.find_element_by_css_selector("#exportWord"))
        except:
            pass
        time.sleep(20)
        filetype = '.pdf'
    ...
    def checkequals(self,datalist1,datalist2):
        if isinstance(datalist1,list) and isinstance(datalist2,list):
            data = list(set(datalist1)-set(datalist2))
            return data
        if isinstance(datalist1,dict) and isinstance(datalist2,dict):
            data = dict(set(datalist1.items())-set(datalist2.items()))
            return data
                ...
                for index,table in enumerate(tables):
                    ...
                    if nrowsdata:
                        if 'null'in nrowsdata or 'default' in nrowsdata or 'null' in nrowsdata:
                            error.append('导出第%s页数据中有错误信息，请查看！'%(index+1))
                        else:
                            print("文件第%s页内容正常！"%(index+1))
                    else:
                        error.append("导出文件第%s页内容为空！"%(index+1))
            else:
                error.append("导出文件后未找到相应的文件！")

    #type为0为导入case，type为1为调用case
    @classmethod
    def importCase(self,type=0):
