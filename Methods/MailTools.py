...
class MailTools:
    ...
    def send_mail(self, reciepents, title, cost, body=None):
        ...
        DEFAULT_MAIL_SENDER = '%s<projecttest@rnbtech.com.hk>' % (Header('BeOP-UI自动化测试', 'utf-8'))
        ...
        ''' + """
           '\n'""" + str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
            time.time()))) + """'\n'""" + '<br/><h2>本次测试耗时%.2f分钟！</h2>' % cost + '<br/>本次测试使用机器 : %s <br/><br/>本次测试使用浏览器版本: <br/>%s' % (
        app.config['MACHINE'], '<br/>'.join(app.config["BROWSER_VERSION"]))
...
if __name__ == '__main__':
    m = MailTools()
    server = m.loginEmail('projecttest@rnbtech.com.hk', 'Rnbtech1103')
