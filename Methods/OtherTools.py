...
class OtherTools:
    ...
    def generateFile(self, caseID, startTime, endTime):
        ...
        try:
            ...
            etStr = datetime.now().strftime("%Y-%m-%d %H:%M:%S").split(' ')[0] + ' ' + endTime
            if endTime=='None':
                etStr=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            st = datetime.strptime(stStr, '%Y-%m-%d %H:%M:%S')
            et = datetime.strptime(etStr, '%Y-%m-%d %H:%M:%S')
            if filenameList!=[]:
                for f in filenameList:
            ...
            print(e.__str__())
            Log.writeLogError(self.logName, 'funcname: generateFile error: %s' % e.__str__())
    ...
    def generateBs64(self, caseId, file):
        ...
        b64pic = base64.b64encode(btContent).decode()
        try:
            r = requests.post(url='http://140.207.49.230:5009/savePic', json={'caseId': caseId, 'filename': file, 'machine': app.config['USER'], 'bsPic': b64pic})
        except:
            return None
        return r.text
